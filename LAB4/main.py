...
from quaternion_operations import q_from_axis_angle, rotate_vector_quaternion
...
def calculate_vertex_normals(data: Dict[str, list], rotate_code) -> np.ndarray:
    vn = np.zeros((len(data["v"]), 3), dtype=np.float32)

    quaternions = np.tile(np.array([1, 0, 0, 0]), (len(data["v"]), 1))

    if rotate_code != 0:
        q_rot = q_from_axis_angle(
            RENDER_CONFIG["axis_for_quater"], RENDER_CONFIG["rotation_angle_for_quater"]
        )
...
def build_model(
    data: Dict[str, list], matrix, z_buff, textures, rotate_code
) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    print(
        "F10 - обработка с поворотом по Эйлеру\n"
    ...
    while True:
        if keyboard.is_pressed("F10"):
            ...
            rotation_matrix = build_rotation_matrix(
                *RENDER_CONFIG["rotation_angles_for_eiler"]
            )
